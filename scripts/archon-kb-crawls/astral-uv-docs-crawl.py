#!/usr/bin/env python3
"""
Astral UV Documentation Crawl Script for Archon
Updated: 2026-01-01

Uses llms.txt for efficient ingestion:
- Single HTTP request to fetch documentation index
- Archon automatically parses sections into separate "pages"
...
def start_crawl() -> str | None:
    """Start crawling the llms.txt file and return progress ID."""
    payload = {
        "url": LLMS_URL,
    ...
    try:
        print(f"Starting crawl: {LLMS_URL}")
        response = httpx.post(
...
def print_summary(result: dict) -> None:
    ...
    if status == "completed":
        print("\nThe Astral UV documentation is now available in Archon!")
...
def main() -> int:
    """Main entry point."""
    print(f"\n{'#'*60}")
    print("# Astral UV Documentation Crawl for Archon")
    print("# Using llms.txt for efficient ingestion")
    print(f"# Target: {LLMS_URL}")
    ...
    print("This will crawl the Astral UV llms.txt file and create a single")
